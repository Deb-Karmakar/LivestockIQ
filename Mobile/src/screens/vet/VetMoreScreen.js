...
const VetMoreScreen = ({ navigation }) => {
    ...
    const menuItems = [
        ...
        },
        {
            ...
            items: [
                { name: t('support'), icon: 'help-circle', screen: 'RaiseTicket', color: theme.primary },
        ...
        <ScrollView style={[styles.container, { backgroundColor: theme.background }]}>
            ...
            <View style={[styles.header, { backgroundColor: theme.card, borderBottomColor: theme.border }]}>
                ...
                <View style={styles.profileInfo}>
                    <Text style={[styles.profileName, { color: theme.text }]}>{user?.fullName || t('veterinarian')}</Text>
                    <Text style={[styles.profileEmail, { color: theme.subtext }]}>{user?.email}</Text>
                    <Text style={[styles.vetId, { color: theme.primary }]}>{t('unique_vet_id')}: {user?.vetId}</Text>
                </View>
            </View>
            ...
            {/* Logout Button */}
            <TouchableOpacity
                style={[styles.logoutButton, { backgroundColor: theme.card, borderColor: `${theme.error}30` }]}
                onPress={logout}
            >
            ...
            <View style={styles.footer}>
                <Text style={[styles.footerText, { color: theme.subtext }]}>{t('version')} 1.0.0</Text>
            </View>
        </ScrollView>
    );
};
...
const styles = StyleSheet.create({
    container: { flex: 1 },
    header: { padding: 24, paddingTop: 60, flexDirection: 'row', alignItems: 'center', borderBottomWidth: 1 },
    profileIcon: { width: 64, height: 64, borderRadius: 32, justifyContent: 'center', alignItems: 'center', marginRight: 16 },
    profileInfo: { flex: 1 },
    profileName: { fontSize: 20, fontWeight: 'bold', marginBottom: 4 },
    profileEmail: { fontSize: 13, marginBottom: 4 },
    vetId: { fontSize: 14, fontWeight: '600' },
    section: { paddingHorizontal: 16, paddingTop: 20 },
    sectionTitle: { fontSize: 13, fontWeight: '600', textTransform: 'uppercase', marginBottom: 8, letterSpacing: 0.5 },
    sectionCard: { borderRadius: 12, overflow: 'hidden', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 3, elevation: 2 },
    ...
    logoutButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginHorizontal: 16, marginTop: 24, marginBottom: 16, padding: 16, borderRadius: 12, borderWidth: 1 },
