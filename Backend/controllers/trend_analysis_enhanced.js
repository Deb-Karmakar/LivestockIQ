// Enhanced getTrendAnalysisData function - adds to regulator.controller.js

export const getTrendAnalysisDataEnhanced = async (req, res) => {
    try {
        // Support time period filtering from query params
        ...
        // Import FeedAdministration model dynamically
        ...
        const [
            ...
            // Query 2: AMU by Species (existing)
            Treatment.aggregate([
                { $match: { status: 'Approved', startDate: { $gte: startDate } } },
                {
                    $lookup: {
            ...
            // Query 4: Feed administrations by month
        ...
        // NEW: Process AMU by method (treatment vs feed)
        ...
        const amuByMethod = {
            data: Array.from(amuByMethodMap.values()).sort((a, b) => {
                if (a.year !== b.year) return a.year - b.year;
                return a.month - b.month;
            })
        };

        // NEW: Process top drugs with WHO risk classification
        const whoCIA = [
            'Colistin', 'Fluoroquinolones', 'Enrofloxacin', 'Ciprofloxacin',
        ...
        const topDrugs = topDrugsRaw.map((drug, index) => {
            ...
            const isHighRisk = whoCIA.some(cia =>
                drugName.toLowerCase().includes(cia.toLowerCase())
            );
            ...
            return {
                ...
                riskLevel: isHighRisk ? 'high' : (index < 3 ? 'medium' : 'low'),
                trend: 'stable'
            };
        });

        // NEW: Process high AMU farms
        const avgTreatmentsPerFarm = farmCount > 0 ? totalAMU / farmCount : 0;
        const highAmuFarms = treatmentsByFarmRaw
            .map(farm => {
                const score = avgTreatmentsPerFarm > 0
                    ? Math.round((farm.count / avgTreatmentsPerFarm) * 100)
                    : 0;

                return {
                    id: farm._id,
                    name: farm.farmInfo.farmName,
                    owner: farm.farmInfo.farmOwner,
                    treatments: farm.count,
                    percentileRank: Math.min(score, 999),
                    status: score > 200 ? 'critical' : score > 150 ? 'high' : 'elevated'
                };
            })
            .filter(farm => farm.percentileRank > 120); // Only farms significantly above average

        // NEW: Summary statistics
        const summary = {
            totalTreatments: totalAMU,
            treatmentOnly: totalTreatments,
            feedOnly: totalFeedAdministrations,
            uniqueDrugs: uniqueDrugs.length,
            avgTreatmentsPerFarm: parseFloat(avgTreatmentsPerFarm.toFixed(1)),
            alertsTriggered: highAmuAlertsCount,
            period: period
        };
