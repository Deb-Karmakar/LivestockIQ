...
export const getDemographicsData = async (req, res) => {
    try {
        const [herdCompositionRaw, ageDistributionRaw] = await Promise.all([
            // Query 1: Group animals by species
            Animal.aggregate([
                { $group: { _id: '$species', count: { $sum: 1 } } },
                { $sort: { count: -1 } }
            ]),
            // Query 2: Group animals into age buckets
            Animal.aggregate([
                {
                    // First, calculate the age of each animal in years
                    $project: {
                        age: {
                            $divide: [
                ...
                {
                    // Next, group them into predefined age buckets
                    $bucket: {
                        groupBy: "$age",
                        boundaries: [0, 1, 3, 5, 10, Infinity], // Age groups: 0-1, 1-3, 3-5, 5-10, 10+
                        default: "Unknown",
                        output: { "count": { $sum: 1 } }
                    }
                }
        ...
        // Process herd composition for the pie chart
        const herdComposition = herdCompositionRaw.map(item => ({
            name: item._id,
            value: item.count
        }));
        
        // Process age distribution for the bar chart
        ...
        res.json({ herdComposition, ageDistribution });
